	...
	const onCommentLike = (comment) => {
		axios.get('sanctum/csrf-cookie').then(() => {
			axios.post(`${props.url}/api/post-comment-likes`, {
				...
				props.setMessage(res.data)
				axios.get(`${props.url}/api/post-comment-likes`).then((res) => props.setPostCommentLikes(res.data))
			}).catch((err) => {
				const resErrors = err.response.data.errors
				var resError
				var newError = []
	...
	return (
		<div className="row">
			...
			<div className="col-sm-4">
				...
				{props.postComments.filter((comment) => comment.post_id == id).map((comment, index) => (
					<div key={index} className='media p-2 border-bottom'>
						...
						<div className='media-body'>
							...
							{/* Post likes */}
							{props.postCommentLikes.find((commentLike) => {
							...
							<small> {props.postCommentLikes.filter((commentLike) => commentLike.comment_id == comment.id).length}</small>
